...
def _hciconfig_up(dev_id: int) -> None:
    """HCI デバイスを UP 状態にする。

    bluetoothd 停止後に DOWN になる場合があるため、ソケット開設前に呼び出す。
    失敗しても継続する（既に UP の場合や hciconfig 非存在の場合を考慮）。

    Args:
        dev_id: HCI デバイス番号。
    """
    subprocess.run(
        ['hciconfig', f'hci{dev_id}', 'up'],
        check=False,
        capture_output=True,
    )
